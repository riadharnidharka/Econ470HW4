...
I am still unsure exactly how to do the code for this question but the steps I would take are to narrow down the data to a small window around the threshold, create a binary variable that indicates whether each plan is above or below the threshold, and then compare the HMO and Part D status for plans just above and below the threshold and create an average for both. NEW: The plans right above and below the threshold are basically the same
